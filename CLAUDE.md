...
- **Ask before committing**: never run `git commit` without asking first, even when a broader request (e.g. "create a branch with these changes") would naturally include committing — confirm each commit individually (see global CLAUDE.md).
...
- **Tagging**: every AWS resource created by this project's Terraform gets the same tag set — at minimum `Project = <project_name>` and `ManagedBy = terraform-<module>` — as established in `terraform/bootstrap/main.tf`. Keep it consistent across the bootstrap and main stacks so cost allocation and resource ownership stay traceable.
